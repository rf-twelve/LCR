                    ...
                    <x-table>
                        <x-slot name="head">
                            ...
                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('work_order_no')"
                                :direction="$sortField === 'work_order_no' ? $sortDirection : null">Work Order #</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('assigned_worker')"
                                :direction="$sortField === 'assigned_worker' ? $sortDirection : null">Assigned Worker</x-table.head>

                            <x-table.head class="w-10 px-2 py-1" sortable wire:click="sortBy('date_started')"
                                :direction="$sortField === 'date_started' ? $sortDirection : null">Date Started</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('date_finished')"
                                :direction="$sortField === 'date_finished' ? $sortDirection : null">Date Finished</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('supervised_by')"
                                :direction="$sortField === 'supervised_by' ? $sortDirection : null">Supervised By</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('approved_by')"
                                :direction="$sortField === 'approved_by' ? $sortDirection : null">Approved By</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('received_by')"
                                :direction="$sortField === 'received_by' ? $sortDirection : null">Received By</x-table.head>
                            ...
                            <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $item->id }}" class="text-gray-600 hover:bg-blue-100">
                                ...
                                <x-table.cell class="max-w-2xl">
                                    <div class="flex justify-center space-x-2">
                                        {{-- View --}}
                                        <a href="{{ route('work-order-slip',['user_id'=>auth()->user()->id,'id'=>$item->id]) }}" class="px-2 py-2 text-sm font-medium text-center text-gray-700 bg-white border border-gray-300 shadow-sm hover:text-white hover:bg-green-500 rounded-xl">
                                            <x-icon.view class="w-5 h-5" /></a>
                                        {{-- Edit --}}
                                        @if ($item->author_id == auth()->user()->id)
                                            <a href="{{ route('work-order-slip.edit',['user_id'=>auth()->user()->id,'id'=>$item->id]) }}" class="px-2 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 shadow-sm hover:text-white hover:bg-blue-500 rounded-xl">
