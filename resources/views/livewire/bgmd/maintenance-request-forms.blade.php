                    ...
                    <x-table>
                        <x-slot name="head">
                            ...
                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('tn')"
                                :direction="$sortField === 'tn' ? $sortDirection : null">Tracking #</x-table.head>
                            ...
                            <x-table.head class="w-10 px-2 py-1" sortable wire:click="sortBy('defects')"
                                :direction="$sortField === 'defects' ? $sortDirection : null">Defects</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('requested_by')"
                                :direction="$sortField === 'requested_by' ? $sortDirection : null">Requested By</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('requested_date')"
                                :direction="$sortField === 'requested_date' ? $sortDirection : null">Requested Date</x-table.head>
                        ...
                        <x-slot name="body">
                            ...
                            <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $item->id }}" class="text-gray-600 hover:bg-blue-100">
                                ...
                                <x-table.cell>
                                    <span class="inline-flex items-baseline px-2.5 py-0.5 rounded-full text-xs uppercase italic font-medium bg-gray-200 text-gray-800 md:mt-2 lg:mt-0">
                                        {{ $item->status }}
                                    </span>
                                </x-table.cell>
                                <x-table.cell>
                                ...
                                <x-table.cell class="max-w-2xl">
                                    <div class="flex justify-center space-x-2">
                                        {{-- View --}}
                                        <a href="{{ route('maintenance-request-form',['user_id'=>auth()->user()->id,'id'=>$item->id]) }}" class="px-2 py-2 text-sm font-medium text-center text-gray-700 bg-white border border-gray-300 shadow-sm hover:text-white hover:bg-green-500 rounded-xl">
                                            <x-icon.view class="w-5 h-5" /></a>
                                        {{-- Edit --}}
                                        @if ($item->author_id == auth()->user()->id)
                                            <a href="{{ route('maintenance-request-form.edit',['user_id'=>auth()->user()->id,'id'=>$item->id]) }}" class="px-2 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 shadow-sm hover:text-white hover:bg-blue-500 rounded-xl">
