        ...
        <x-header.navbar>
            <nav class="flex" aria-label="Breadcrumb">
                ...
                <li>
                    ...
                    <a href="{{ route('equipments',['user_id'=>Auth::user()->id]) }}" class="ml-4 text-sm font-medium hover:text-blue-100" aria-current="page">Equipment</a>
                    </div>
                </li>
                        ...
                        <div class="flex-shrink-0">
                            <div class="flex justify-between px-6 py-2 text-sm font-medium text-gray-700 bg-blue-300 border-t border-b border-gray-200">
                                <div class="flex">
                                    ...
                                    <span class="pl-2">VEHICLE INFO</span>
                                    @if ($author_id == auth()->user()->id)
                                        <a href="{{ route('vehicle.edit',['user_id'=>auth()->user()->id, 'id'=> $vehicle_id]) }}" class="flex ml-3 text-indigo-600 hover:text-indigo-900">
                            ...
                            <div class="sm:items-baseline sm:justify-between">
                                ...
                                <div class="flex justify-between px-6 py-2 text-sm font-medium text-gray-700 bg-blue-300 border-t border-b border-gray-200">
                                    <div class="flex">
                                        ...
                                        <span class="pl-2">VEHICLE MAINTENANCE LOG</span>
                                        <a href="{{ route('vehicle-maintenance-log.print',['user_id'=>auth()->user()->id, 'id'=> $vehicle_id]) }}" terge="__blank" class="flex ml-3 text-indigo-600 hover:text-indigo-900">
                                            ...
                                            <x-slot name="head">
                                                <thead class="px-3 text-sm text-center text-gray-900 bg-gray-300 border border-gray-50">
                                                    <tr class="font-semibold">
                                                        <th class="text-center border">DATE OF SERVICE</th>
                                                        <th class="text-center border">MILEAGE AT SERVICE</th>
                                                        <th class="text-center border">WORK PERFORMED AND SERVICES SCHEDULE</th>
                                                        <th class="text-center border">PERFORMED BY</th>
                                                        <th class="text-center border">COST</th>
                                                        <th class="text-center border">NOTES/REMARKS</th>
                                                    </tr>
                                                </thead>
                                            </x-slot>

                                            <x-slot name="body">
                                                @forelse ($maintenance_log as $key => $log)
                                                    <tr class="px-3 py-2 text-gray-500 border whitespace-nowrap">
                                                        <td class="px-3 py-2 border">{{ $log['service_date'] }}</td>
                                                        <td class="px-3 py-2 border">{{ $log['service_mileage'] }}</td>
                                                        <td class="px-3 py-2 border">{{ $log['work_performed'.''.'services_schedule'] }}</td>
                                                        <td class="px-3 py-2 border">{{ $log['performed_by'] }}</td>
                                                        <td class="px-3 py-2 border">{{ $log['cost'] }}</td>
                                ...
                                <div class="flex flex-col">
                                    <div class="min-w-full overflow-hidden overflow-x-scroll align-middle shadow">
                                        @forelse ($vehicle_images as $image)
