                    ...
                    <x-table>
                        <x-slot name="head">
                            ...
                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('make')"
                                :direction="$sortField === 'make' ? $sortDirection : null">Type/Make</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('brand')"
                                :direction="$sortField === 'brand' ? $sortDirection : null">Brand</x-table.head>

                            <x-table.head class="w-10 px-2 py-1" sortable wire:click="sortBy('brand')"
                                :direction="$sortField === 'brand' ? $sortDirection : null">Brand</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('year')"
                                :direction="$sortField === 'year' ? $sortDirection : null">Year</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('plate_no')"
                                :direction="$sortField === 'plate_no' ? $sortDirection : null">Plate No.</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('serial_no')"
                                :direction="$sortField === 'serial_no' ? $sortDirection : null">Serial No.</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('acquisition_date')"
                                :direction="$sortField === 'acquisition_date' ? $sortDirection : null">Acquisition Date</x-table.head>

                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('acquisition_cost')"
                                :direction="$sortField === 'acquisition_cost' ? $sortDirection : null">Acquisition Cost</x-table.head>
                            ...
                            <x-table.head class="px-2 py-1" sortable wire:click="sortBy('remarks')"
                                :direction="$sortField === 'remarks' ? $sortDirection : null">Remarks</x-table.head>
                            ...
                            <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $item->id }}" class="text-gray-600 hover:bg-blue-100">
                                ...
                                <x-table.cell class="max-w-2xl">
                                    <div class="flex justify-center space-x-2">
                                        {{-- View --}}
                                        <a href="{{ route('equipment',['user_id'=>auth()->user()->id,'id'=>$item->id]) }}" class="px-2 py-2 text-sm font-medium text-center text-gray-700 bg-white border border-gray-300 shadow-sm hover:text-white hover:bg-green-500 rounded-xl">
                                            <x-icon.view class="w-5 h-5" /></a>
                                        {{-- Edit --}}
                                        @if ($item->author_id == auth()->user()->id)
                                            <a href="{{ route('equipment.edit',['user_id'=>auth()->user()->id,'id'=>$item->id]) }}" class="px-2 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 shadow-sm hover:text-white hover:bg-blue-500 rounded-xl">
